...
impl Connection {
    ...
    pub async fn call(&mut self, method: &str, params: Value) -> Result<Value> {
        ...
        if let Some(err) = response.get("error") {
            let msg = err.get("message")
                ...
                .to_string();
            return Err(CdpError::CommandFailed { method: method.to_string(), msg });
        }
    ...
    pub async fn call_with_session(&mut self, session_id: &str, method: &str, params: Value) -> Result<Value> {
        let id = self.next_id.fetch_add(1, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_cdp_error_response() {
        let response = json!({"id": 2, "error": {"code": -32000, "message": "Cannot find context"}});
        assert!(response.get("error").is_some());
        assert_eq!(response["error"]["message"].as_str(), Some("Cannot find context"));
    }
