...
impl Tab {
    ...
    pub async fn create(conn: &mut Connection, ws_url: &str, url: &str) -> Result<Self> {
        ...
        let result = conn.call("Target.createTarget", json!({
            "url": url,
            "newWindow": false,
        })).await;
        ...
        if let Ok(result) = result {
            if let Some(session_id) = result.get("sessionId").and_then(|v| v.as_str()) {
                ...
                return Ok(Tab { session_id: session_id.to_string(), target_id });
            }
            // Got targetId but no sessionId (e.g. reusing persistent browser) —
            // fall through to HTTP method to attach
            tracing::warn!("Target.createTarget returned targetId without sessionId, falling back to HTTP");
        } else {
    ...
    async fn create_via_http(conn: &mut Connection, ws_url: &str, url: &str) -> Result<Self> {
        ...
        let target_info: Value = resp.json().await
            .map_err(|e| CdpError::CommandFailed {
                method: "create_via_http".into(),
                msg: format!("HTTP response parse failed: {e}"),
            })?;
        
        let target_id = target_info.get("id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| CdpError::CommandFailed {
                method: "create_via_http".into(),
        ...
        let attach_result = conn.call("Target.attachToTarget", json!({
            "targetId": target_id,
            "flatten": true,
        })).await?;
        ...
        tracing::info!("Attached to created target: {} (session={})", target_id, session_id);
        
        Ok(Tab { session_id, target_id })
    }
    ...
    pub async fn navigate(&self, conn: &mut Connection, url: &str) -> Result<()> {
        ...
        let result = conn.call_with_session(&self.session_id, "Page.enable", json!({})).await?;
        tracing::debug!("Page.enable: {:?}", result);
        
        // Navigate
        let _result = conn.call_with_session(&self.session_id, "Page.navigate", json!({
            "url": url,
        })).await?;
    ...
    async fn wait_for_page_load(&self, conn: &mut Connection, timeout_ms: u64) -> Result<()> {
        ...
        loop {
            ...
            let result = conn.call_with_session(&self.session_id, "Runtime.evaluate", json!({
    ...
    pub async fn extract_text(&self, conn: &mut Connection) -> Result<String> {
        let result = self.evaluate(conn, "document.body?.innerText || ''").await?;
    ...
    pub async fn extract_html(&self, conn: &mut Connection) -> Result<String> {
        let result = self.evaluate(conn, "document.documentElement?.outerHTML || ''").await?;
    ...
    pub async fn close(self, conn: &mut Connection) {
        ...
        let _ = conn.call_with_session(&self.session_id, "Runtime.evaluate", json!({
        ...
        let _ = conn.call("Target.closeTarget", json!({
