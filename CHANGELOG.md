...
- Fix daemon child process surviving parent exit (process_group(0), Stdio::null(), readiness polling)
- Add drain() to SessionPool to close idle tabs after batch operations
- Fix race condition between background checkin and pool.drain() via pending_checkins counter
- Fix handle_eval tab close — now uses Dia quirk (window.close → Target.closeTarget)
- Fix handle_close_tab — attaches session and calls window.close() before Target.closeTarget
- Call pool.drain() after handle_search_batch, handle_follow_batch, handle_harvest
- Add interactive create-changeset.sh helper script
- Fix CHANGELOG.md spacing consistency
- Update VERSION.md docs with corrected examples and formatter note
